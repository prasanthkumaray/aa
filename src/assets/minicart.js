...
Minicart.prototype = {
    ...
    removeItem: function(el) {
        ...
        $j.ajax({
            ...
            if (result.success) {
                ...
                var blockQty = $j('.skip-cart .quantity');
                var blockQty2 = $j('.skip-cart .quantity-mobile');
                if(result.qty>0){
                    blockQty.replaceWith('<span class="quantity"> (' +result.qty+') </span>');
                    blockQty2.replaceWith('<span class="quantity-mobile"> (' +result.qty+') </span>');
                }else{
                    blockQty.replaceWith('<span class="quantity"> </span>');
                    blockQty2.replaceWith('<span class="quantity-mobile"> </span>');
                }
            } else {
    ...
    showOverlay: function() {
        $j('#mini-cart-loader').stop(true, true).fadeIn();
        //$j(this.selectors.overlay).addClass('loading');
    },

    hideOverlay: function() {
        $j('#mini-cart-loader').stop(true, true).fadeOut();
